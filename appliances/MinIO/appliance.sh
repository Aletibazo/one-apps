...
in the template contextualization, the MinIO Environment Variable File will use the defaults defined [here](https://min.io/docs/minio/linux/operations/install-deploy-manage/deploy-minio-single-node-single-drive.html#create-the-environment-variable-file).
...
the configure script will use that partition to mount on the corresponding path.

After the VM has booted and app
EOF
)
...
service_configure()
{
    ...
    minio_drives=$(lsblk -d -o NAME | grep vd | grep -v 'vda')
    drives_count=1
    MINIO_VOLUMES=""
    for device in $minio_drives; do
        # Partition and format dev
        partition_format_drive $device
        # Create folder in /mnt for drive mount
        folder="/mnt/disk-${drives_count}"
        MINIO_VOLUMES+=" ${folder}"
    ...
    [[ ! $(mount -a -t xfs) ]] || msg error "Error mounting MinIO drives"

    msg info "Give ownership to minio-user on ${MINIO_VOLUMES}"
    chown -R minio-user:minio-user ${MINIO_VOLUMES}

    # Edit MinIO environment variable file
    if (( drives_count > 1 )); then
        update_environment_variable_file "/mnt/disk-{1...$((drives_count))}"
    else
...
partition_format_drive()
{
    ...
    if [[ -z "$(lsblk -nd -o PTTYPE /dev/${1})" ]]; then 
        ...
        sed -e 's/\s*\([\+0-9a-zA-Z]*\).*/\1/' << EOF | fdisk /dev/${1}
